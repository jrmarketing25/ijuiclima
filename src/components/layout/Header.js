...
export function renderHeader() {
  ...
  return `
    ...
    <aside class="bg-[#0C111A] text-slate-300 text-xs py-1.5 sm:py-2 px-4 border-b border-white/5" aria-label="Informações de atendimento">
      <div class="max-w-7xl mx-auto flex justify-between items-center gap-2">
        <div class="flex items-center gap-2">
          <span class="inline-block w-1.5 h-1.5 rounded-full bg-[#0090D0] animate-pulse shrink-0"></span>
          <span class="tracking-wide sm:hidden font-medium text-slate-200">Ijuí e região</span>
          <span class="tracking-wide hidden sm:inline">Atendimento especializado em <strong>${company.locationString}</strong> e região</span>
        </div>
    ...
    <header id="main-header" class="sticky top-0 z-40 w-full transition-all duration-400 ease-[cubic-bezier(0.22,1,0.36,1)] py-3.5 sm:py-5 lg:py-7 border-b border-transparent">
      <div class="max-w-7xl mx-auto px-5 max-[375px]:px-4 sm:px-8 flex items-center justify-between">
        ...
        <a href="#" class="flex items-center gap-3 group focus:outline-none rounded-md py-1" aria-label="${company.name} - Início">
          <div class="h-11 sm:h-14 lg:h-[4.25rem] flex items-center">
